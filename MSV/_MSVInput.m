frame(ALWAYS,AND_1,EQUIV,EVENTUALLY,FALSE,IMPLIES,NOT,OR_1,PREDICATE,TRUE,U_OPER,V_OPER,tl_stats,tl_simp_log,tl_simp_diff,tl_simp_fly,tl_simp_scc,tl_fjtofj,tl_errs,tl_verbose,tl_terse,All_Mem,uform,hasuform,cnt,ltl_file,add_ltl,out1,label,sym_table,transition,final_set,node_id,sym_id,node_size,sym_size,astate_count,atrans_count,bstack,bstates,bremoved,scc_stack,accept,bstate_count,btrans_count,stored,Caches,CacheHits,gstack,gremoved,gstates,init,init_size,gstate_id,gstate_count,gtrans_count,fin$,final,rank,scc_id,scc_size,bad_scc,symtab,yytext,atrans_list,gtrans_list,btrans_list,aallocs,afrees,apool,gallocs,gfrees,gpool,ballocs,bfrees,bpool,freelist,req,event,tl_yychar,tl_yylval,prec,can,mod,Stack_mx,Max_Red,Total,dumpbuf,Bscc_stack) and (
                         ...
                         while(Canonical_p)
                         {
                             ...
                             if(Canonical_p=Canonical_m OR Canonical_p->ntyp=-1 OR Canonical_m->ntyp=-1) then 
                             {
                                 ...
                                 if(continue_1=0)   then 
                                 {
                                     ...
                                     if(continue_1=0)   then 
                                     {
                                         if(anywhere(OR_1,Canonical_k1,Canonical_k2,RValue)) then 
                                         {
                                             marknode(AND_1,Canonical_p);
                                             continue_1<==1 and skip;
                                              Canonical_p:=( if((Canonical_p->ntyp=AND_1)) then Canonical_p->rgt else NULL)
                                         ...
                                         if(continue_1=0)   then 
                                         {
                                             if(Canonical_k2->ntyp=U_OPER AND anywhere(AND_1,Canonical_k2->rgt,can,RValue)) then 
                                             {
                                                 marknode(AND_1,Canonical_p);
                                                 continue_1<==1 and skip;
                                                  Canonical_p:=( if((Canonical_p->ntyp=AND_1)) then Canonical_p->rgt else NULL)
                                 ...
                                 while(Canonical_p)
                                 {
                                     ...
                                     if(Canonical_p=Canonical_m OR Canonical_p->ntyp=-1 OR Canonical_m->ntyp=-1) then 
                                     {
                                         ...
                                         if(continue_1=0)   then 
                                         {
                                             ...
                                             if(continue_1=0)   then 
                                             {
                                                 if(anywhere(AND_1,Canonical_k1,Canonical_k2,RValue)) then 
                                                 {
                                                     marknode(OR_1,Canonical_p);
                                                     continue_1<==1 and skip;
                                                      Canonical_p:=( if((Canonical_p->ntyp=OR_1)) then Canonical_p->rgt else NULL)
                                                 ...
                                                 if(continue_1=0)   then 
                                                 {
                                                     if(Canonical_k2->ntyp=V_OPER AND Canonical_k2->lft->ntyp=FALSE AND anywhere(AND_1,Canonical_k2->rgt,can,RValue)) then 
                                                     {
                                                         marknode(OR_1,Canonical_p);
                                                         continue_1<==1 and skip;
                                                          Canonical_p:=( if((Canonical_p->ntyp=OR_1)) then Canonical_p->rgt else NULL)
  ...
  function dump_cond ( Node *pp,Node *r,int first,int RValue )
 {
     ...
     if(return_1=0)   then 
     {
         ...
         {
             if(dump_cond_q->ntyp=V_OPER AND !anywhere(AND_1,dump_cond_q->rgt,r,RValue)) then 
             {
