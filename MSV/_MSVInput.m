frame(ALWAYS,AND_1,EQUIV,EVENTUALLY,FALSE,IMPLIES,NOT,OR_1,PREDICATE,TRUE,U_OPER,V_OPER,tl_stats,tl_simp_log,tl_simp_diff,tl_simp_fly,tl_simp_scc,tl_fjtofj,tl_errs,tl_verbose,tl_terse,All_Mem,uform,hasuform,cnt,ltl_file,add_ltl,out1,label,sym_table,transition,final_set,node_id,sym_id,node_size,sym_size,astate_count,atrans_count,bstack,bstates,bremoved,scc_stack,accept,bstate_count,btrans_count,stored,Caches,CacheHits,gstack,gremoved,gstates,init,init_size,gstate_id,gstate_count,gtrans_count,fin$,final,rank,scc_id,scc_size,bad_scc,symtab,yytext,atrans_list,gtrans_list,btrans_list,aallocs,afrees,apool,gallocs,gfrees,gpool,ballocs,bfrees,bpool,freelist,req,event,tl_yychar,tl_yylval,prec,can,mod,Stack_mx,Max_Red,Total,dumpbuf,Bscc_stack) and (
 ...
 {
     frame(main_i,main_1_formula,return_1) and ( 
     int return_1<==0 and skip;
     int main_i and skip;
     if(ltl_file) then 
     {
  ...
  function in_cache ( Node *n,Node* RValue )
 {
     ...
     while( return_1=0 AND  in_cache_d)
     {
         if(extern isequal(in_cache_d->before,n,RValue)) then 
         {
             ...
             if(in_cache_d->same AND (extern ismatch(n,in_cache_d->before,RValue))) then 
             {
  ...
  function simplify_gtrans ( int RValue )
 {
     ...
     while(simplify_gtrans_s!=gstates)
     {
         ...
         while(simplify_gtrans_t!=simplify_gtrans_s->trans)
         {
             ...
             while(!((simplify_gtrans_t!=simplify_gtrans_t1) AND (simplify_gtrans_t1->to=simplify_gtrans_t->to) AND (extern included_set(simplify_gtrans_t1->pos,simplify_gtrans_t->pos,1,RValue)) AND (extern included_set(simplify_gtrans_t1->neg,simplify_gtrans_t->neg,1,RValue)) AND ((externincluded_set(simplify_gtrans_t->final,simplify_gtrans_t1->final,0,RValue)) OR (tl_simp_scc AND ((simplify_gtrans_s->incoming!=simplify_gtrans_t->to->incoming) OR (extern in_set(bad_scc,simplify_gtrans_s->incoming,RValue)))))))
             {
 ...
 {
     frame(addcan_m,addcan_prev,addcan_ptr,addcan_N,addcan_s,addcan_t,addcan_cmp,return_1,goto) and ( 
     int goto<==0 and skip;
     int return_1<==0 and skip;
 ...
 function Canonical ( Node *n,Node* RValue )
 {
     frame(Canonical_m,Canonical_p,Canonical_k1,Canonical_k2,Canonical_prev,Canonical_dflt,Canonical_tok,return_1,goto,continue_1) and ( 
     int continue_1<==0 and skip;
     int goto<==0 and skip;
     int return_1<==0 and skip;
