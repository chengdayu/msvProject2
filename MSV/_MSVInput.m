frame(mDecodeVar) and (
 function memcpy(unsigned char *des, unsigned char *src, int num,char *RValue)
 {
         frame(i) and (
         int i<==0 and skip;
         while(i<num)
  ...
  function DecodeRemoveBlk ( unsigned char *startAddr,int winPos,int blkNum,int winRemoveNum )
 {
     ...
     {
         memcpy(&startAddr[(DecodeRemoveBlk_i-winRemoveNum)*512],&startAddr[DecodeRemoveBlk_i*512],512,RValue);
  ...
  function DecodeLenUpdate ( int lastIdx,int winPos,int winSize,int blkNum,int newLen[],int RValue )
 {
     ...
     while( break_1=0 and  DecodeLenUpdate_i<winSize)
     {
         if(lastIdx>512) then 
         {
             newLen[winPos+DecodeLenUpdate_i]:=512;
             lastIdx:=lastIdx-512
  ...
  function DecodeRemoveByteBlk ( unsigned char buffer[],int len$,int delelteIdx[],int RValue )
 {
     ...
     DecodeRemoveByteBlk_delCnt:=DecodeFindIndexDelete(buffer,512,delelteIdx,RValue);
  ...
  function DecodeDataMerge ( unsigned char *startAddr,int winPos,int *blkNum,int newLen[],int RValue )
 {
     ...
     {
         DecodeDataMerge_winEleCnt:=DecodeWinMerge(&startAddr[winPos*512],512,DecodeDataMerge_tmpWinSize,&newLen[winPos],RValue);
  ...
  function DecodeEntry (  )
 {
     ...
     if (nm_1$=21845 or (switch_1=1 and break_1$=0 and return_1=0) ) then 
     {
         ...
         {
             mDecodeVar.blkLen[mDecodeVar.blkIdx+DecodeEntry_i]:=DecodeRemoveByteBlk(&mDecodeVar.decodeAddr[(mDecodeVar.blkIdx+DecodeEntry_i)*512],512,mDecodeVar.delelteIdx,RValue);
  ...
  function TestCmdE2DataMerge (  )
 {
     ...
     int break_1<==0 and skip;
     int return_1<==0 and skip;
     int TestCmdE2DataMerge_len08<==512*1024 and skip;
     unsigned char TestCmdE2DataMerge_buffer[1024,512] and skip;
     int TestCmdE2DataMerge_blkNum<==1024 and skip;
     ...
     {
         output (((unsigned char *)TestCmdE2DataMerge_buffer)[TestCmdE2DataMerge_i]) and skip;
